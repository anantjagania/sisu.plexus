...
public class PlexusXmlScannerTest {
    ...
    public void testLoadOnStart() {
        ...
        Assertions.assertEquals(2, componentMap.size());

        final Component component1 =
          new ComponentImpl(DefaultBean.class, Hints.DEFAULT_HINT, Strategies.LOAD_ON_START, "");
        Assertions.assertEquals(DefaultBean.class, componentMap.get(component1).load());

        final Component component2 =
          new ComponentImpl(Bean.class, "debug", Strategies.LOAD_ON_START, "For debugging");
        Assertions.assertEquals(DebugBean.class, componentMap.get(component2).load());
    }
    ...
    public void testComponents() {
        ...
        Assertions.assertEquals(6, componentMap.size());
        ...
        Assertions.assertEquals(AnotherBean.class, componentMap.get(component3).load());

        final Component component4 = new ComponentImpl(Bean.class, "clone", Strategies.SINGLETON, "");
        Assertions.assertEquals(DefaultBean.class, componentMap.get(component4).load().getSuperclass());
        final Class<?> proxy = CustomTestClassLoader.proxy(componentMap.get(component4).load());

        try {
            Assertions.assertNotNull(proxy.getMethod("TestMe"));
        } catch (final NoSuchMethodException e) {
            Assertions.fail("Proxied class is missing 'TestMe' method");
        }

        final PlexusBeanMetadata metadata1 = metadata.get(DefaultBean.class.getName());

        Assertions.assertFalse(metadata1.isEmpty());

        Assertions.assertEquals(new ConfigurationImpl("someFieldName",
            "<some-field-name><item>PRIMARY</item></some-field-name>"),
          metadata1.getConfiguration(new NamedProperty("someFieldName")));

        Assertions.assertEquals(new ConfigurationImpl("simple", "value"),
          metadata1.getConfiguration(new NamedProperty("simple")));

        Assertions.assertEquals(new ConfigurationImpl("value", "<value with=\"attribute\"></value>"),
          metadata1.getConfiguration(new NamedProperty("value")));

        Assertions.assertEquals(new ConfigurationImpl("emptyValue1", "<empty-value1 with=\"attribute\" />"),
          metadata1.getConfiguration(new NamedProperty("emptyValue1")));

        Assertions.assertEquals(new ConfigurationImpl("emptyValue2", ""),
          metadata1.getConfiguration(new NamedProperty("emptyValue2")));

        Assertions.assertFalse(metadata1.isEmpty());

        Assertions.assertEquals(new RequirementImpl(Bean.class, true, "debug"),
        ...
        Assertions.assertEquals(new RequirementImpl(Bean.class, false, Hints.DEFAULT_HINT, "debug"),
          metadata1.getRequirement(new NamedProperty("beanMap")));

        Assertions.assertFalse(metadata1.isEmpty());

        Assertions.assertEquals(new RequirementImpl(Bean.class, false),
          metadata1.getRequirement(new NamedProperty("beanField")));

        Assertions.assertTrue(metadata1.isEmpty());

        Assertions.assertNotNull(metadata.get(AnotherBean.class.getName()));
        Assertions.assertNull(metadata.get(DebugBean.class.getName()));
    }
    ...
    public void testBadComponentsXml() {
        ...
        try {
            ...
            badReq.role();
            Assertions.fail("Expected TypeNotPresentException");
        ...
        Assertions.assertTrue(scanner.scan(space, true).isEmpty());
    }
    ...
    public void testInterpolatedComponentsXml() {
        ...
        Assertions.assertEquals("INTERPOLATED",
          metadata.get(DefaultBean.class.getName()).getConfiguration(new NamedProperty("variable")).value());
    }
    ...
    public void testLocalizedXmlScanning() {
        ...
        Assertions.assertFalse(new PlexusXmlScanner(null, null, null).scan(space, true).isEmpty());
        Assertions.assertTrue(new PlexusXmlScanner(null, null, null).scan(space, false).isEmpty());
    }
